...
async function handleRequest(request) {
  ...
  const templateUrl = 'https://templates.xyhk.us.kg/singbox';
...
function formatProxies(data) {
  return data.map(({ area = '未知区域', title = '未命名', ip, port, password, type = 'shadowsocks'}) => ({
    area, title, ip, port, password, type
  }));
}
...
async function generateClashConfig(data, templateUrl) {
  ...
  const proxies = data.map(({ area, title, ip, port, type, password }) =>
    `{"type":"${type}","tag":"${area}-${title}","method":"aes-256-cfb","password":"${password}","server":"${ip}","server_port":${port}}`
  );

  const proxyNames = data.map(({ area, title }) => `"${area}-${title}"`);
